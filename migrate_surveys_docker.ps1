...
if ($runningServices -split "`r?`n" -contains "flask") {
    Write-Host "Running migrate_project_surveys.py inside running 'flask' container..."
    & ($ComposeCmd.Split()[0]) ($ComposeCmd.Split()[1..($ComposeCmd.Split().Length-1)] + @("exec", "flask", "python", "migrate_project_surveys.py") + $args)
} else {
    Write-Host "'flask' container is not running. Running via temporary container..."
    & ($ComposeCmd.Split()[0]) ($ComposeCmd.Split()[1..($ComposeCmd.Split().Length-1)] + @("run", "--rm", "flask", "python", "migrate_project_surveys.py") + $args)
}
